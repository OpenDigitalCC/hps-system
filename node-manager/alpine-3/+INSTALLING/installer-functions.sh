...
# -------------------------------
# Configure persistent network interfaces for installed system.
#
# Behaviour:
#   - Detects management interface (eth0 or first available)
#   - Creates /etc/network/interfaces with DHCP config
#   - Enables networking service in default runlevel
#   - Must be called early before services that need network
#
# Arguments:
#   $1 - target_root (optional, defaults to "" for live system, "/mnt" for installer)
#
# Returns:
#   0 on success
#   1 if failed to detect interface
#   2 if failed to write config
#
# Example usage:
#   n_configure_persistent_network          # Configure live system
#   n_configure_persistent_network /mnt     # Configure installed system at /mnt
#
#===============================================================================
n_configure_persistent_network() {
  local target_root="${1:-}"
  local interfaces_file="${target_root}/etc/network/interfaces"
  local runlevels_dir="${target_root}/etc/runlevels/default"
  
  n_remote_log "[INFO] Configuring persistent network"
  
  # Ensure target directory exists
  mkdir -p "$(dirname "$interfaces_file")"
  mkdir -p "$runlevels_dir"
  
  # Detect management interface - use eth0 as default for SCH/TCH
  local mgmt_iface="eth0"
  
  n_remote_log "[DEBUG] Management interface: $mgmt_iface"
  
  # Create interfaces file with DHCP
  n_remote_log "[INFO] Creating $interfaces_file"
  
  cat > "$interfaces_file" << EOF
# /etc/network/interfaces
# Generated by HPS installer
# Management network - DHCP from IPS
...
EOF
  ...
  if [[ ! -f "$interfaces_file" ]]; then
    n_remote_log "[ERROR] Failed to create $interfaces_file"
    return 2
  fi
  
  n_remote_log "[DEBUG] Created $interfaces_file"
  
  # Enable networking service
  if [[ ! -e "${runlevels_dir}/networking" ]]; then
    ln -s /etc/init.d/networking "${runlevels_dir}/networking" 2>/dev/null || true
    n_remote_log "[DEBUG] Enabled networking service in default runlevel"
  fi
  
  n_remote_log "[INFO] Persistent network configuration complete"
  return 0
}
...
n_installer_detect_target_disks() {
  ...
  if [[ $disk_clean -eq 0 ]]; then
    ...
    echo "" >&2
    echo "===============================================================================" >&2
    echo "ERROR: Target disk(s) contain existing data" >&2
    echo "===============================================================================" >&2
    echo "" >&2
    echo "Installation cannot proceed - existing data may cause boot failures" >&2
    echo "or data loss." >&2
    echo "" >&2
...
n_remote_log "[DEBUG] Checking ext4 filesystem support"

# Check if ext4 already available
if grep -q ext4 /proc/filesystems 2>/dev/null; then
  n_remote_log "[DEBUG] ext4 already available in /proc/filesystems"
else
  n_remote_log "[INFO] ext4 not available, loading modules"
  
  # Verify modloop is mounted (required for module loading)
  if ! mountpoint -q /.modloop 2>/dev/null; then
    n_remote_log "[ERROR] Modloop not mounted at /.modloop"
    n_remote_log "[ERROR] Cannot load ext4 modules - modloop required"
    return 2
  fi
  
  n_remote_log "[DEBUG] Modloop mounted, calling n_installer_load_ext4_modules"
  
  local ext4_output
  ext4_output=$(n_installer_load_ext4_modules 2>&1)
  local ext4_rc=$?
  
  if [[ $ext4_rc -ne 0 ]]; then
    n_remote_log "[ERROR] n_installer_load_ext4_modules failed (exit code: $ext4_rc)"
    echo "$ext4_output" | while IFS= read -r line; do
      n_remote_log "[ERROR] ext4_load: $line"
    done
    return 2
  fi
  ...
  # Verify ext4 is now available
  if ! grep -q ext4 /proc/filesystems 2>/dev/null; then
    n_remote_log "[ERROR] ext4 still not available after module loading"
    n_remote_log "[DEBUG] /proc/filesystems contents:"
    cat /proc/filesystems 2>&1 | while IFS= read -r line; do
      n_remote_log "[DEBUG]   $line"
    done
    return 2
  fi
...
n_installer_install_hps_init() {
  ...
  n_remote_log "[INFO] Configuring persistent network"
  n_configure_persistent_network /mnt
