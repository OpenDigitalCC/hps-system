...
create_config_opensvc() {
  ...
  local tmp; tmp="$(mktemp "${conf_file}.XXXXXX")" || { hps_log error "[opensvc] mktemp failed"; return 1; }
  if ! generate_opensvc_conf "${ips_role}" > "${tmp}"; then
    hps_log error "[opensvc] generate_opensvc_conf failed"; rm -f "${tmp}"; return 1
  fi
  if [[ -s "${conf_file}" ]]; then
  ...
  if [[ -n "${cluster_key}" ]]; then
    ...
    if [[ -n "${disk_key}" && "${disk_key}" != "${cluster_key}" ]]; then
      hps_log error "[opensvc] Refusing to overwrite ${key_file}: differs from cluster OPENSVC_AGENT_KEY"
      hps_log error "[opensvc] disk_key=$(printf '%.8s' "${disk_key}")… cluster_key=$(printf '%.8s' "${cluster_key}")…"
      hps_log error "[opensvc] Resolve mismatch: update cluster_config or replace ${key_file} manually, then rerun."
      return 2
    fi
    # Write/normalize the disk key to the cluster value
    printf '%s\n' "${cluster_key}" > "${key_file}"
    chmod 0600 "${key_file}"; chown root:root "${key_file}"
    hps_log info "[opensvc] Applied cluster OPENSVC_AGENT_KEY to ${key_file}"
  else
    # No cluster key yet → adopt or generate, then persist to cluster_config
    if [[ -n "${disk_key}" ]]; then
      cluster_config set OPENSVC_AGENT_KEY "${disk_key}"
      hps_log info "[opensvc] Adopted existing ${key_file} into cluster_config OPENSVC_AGENT_KEY"
    else
      local new_key
      ...
      printf '%s\n' "${new_key}" > "${key_file}"
      chmod 0600 "${key_file}"; chown root:root "${key_file}"
      hps_log info "[opensvc] Generated and stored new OPENSVC_AGENT_KEY; wrote ${key_file}"
    fi
  fi
  ...
  hps_log info "[opensvc] wrote ${conf_file}; key policy enforced"
}
