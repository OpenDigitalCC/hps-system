...
tch_apkovol_create() {
  ...
  if ! _apkovl_create_alpine_repos_script "$tmp_dir" "$gateway_ip" "$alpine_version"; then
    rm -rf "$tmp_dir"
    return 1
  fi
...
EOF
  ...
  hps_log debug "Alpine repository script created"
  return 0
}
