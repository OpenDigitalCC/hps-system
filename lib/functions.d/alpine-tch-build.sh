...
tch_apkovol_create() {
  ...
  if ! _apkovl_create_modloop_setup "$tmp_dir" "$download_base"; then
    rm -rf "$tmp_dir"
    return 1
  fi
  ...
  # Create queue run script
  if ! _apkovl_create_queue_run_script "$tmp_dir"; then
    rm -rf "$tmp_dir"
    return 1
  fi
...
#
# Arguments:
#   $1 - tmp_dir        : Root directory of apkovl structure
#   $2 - download-base  : Base URL for downloading files
#
# Behaviour:
...
_apkovl_create_modloop_setup() {
  ...
  local setup_script="$tmp_dir/etc/local.d/modloop-setup.start"
  
  # Define modules to load WITH DEPENDENCIES IN ORDER
  local modules=(
    # Core dependencies first
    "mrp"           # Multiple Registration Protocol (for 8021q)
    "mdio"          # MDIO bus (for PHY drivers)
    "mdio-devres"   # MDIO device resource management (for devm_* functions)
    "libphy"        # PHY library (for network drivers needing PHY)
    "i2c-core"      # I2C core (for igb)
    "i2c-algo-bit"  # I2C bit-banging (for igb)
    "dca"           # Direct Cache Access (for igb, ixgbe)
    "hwmon"         # Hardware monitoring (for temperature sensors)
    ...
    # Network drivers (these should now load without errors)
  ...
  hps_log debug "Creating modloop setup script for ${#modules[@]} module(s)"
  
  # Create the modloop setup script
  cat > "$setup_script" <<EOF
#!/bin/sh
# TCH Modloop Setup - Download and mount kernel modules
# Generated by hps-system tch_apkovl_create

MODLOOP_URL="${download_base}/boot/modloop-lts"
...
ln -sf "\$MODULE_DIR" "/lib/modules/\$KVER"

# Load modules with dependency tracking
EOF

  # Add module loading with better error handling
  for module in "${modules[@]}"; do
    cat >> "$setup_script" <<EOF
# Load ${module}
MODULE_FILE=\$(find "\$MODULE_DIR" -name "${module}.ko*" 2>/dev/null | head -n1)
if [ -n "\$MODULE_FILE" ]; then
  if lsmod | grep -q "^${module} "; then
    echo "Module already loaded: ${module}"
  else
    echo -n "Loading module: ${module}... "
    if insmod "\$MODULE_FILE" 2>/tmp/${module}.err; then
      echo "OK"
      sleep 0.1  # Small delay for module initialization
    else
      echo "FAILED"
      if [ -s "/tmp/${module}.err" ]; then
        echo "  Error: \$(cat /tmp/${module}.err)"
      fi
    fi
  fi
else
  # Module might be built-in or not needed for this hardware
  :
fi
...
EOF
  ...
  # Add completion and interface listing
  cat >> "$setup_script" <<'EOF'
# List network interfaces
echo "Network interfaces detected:"
ip link show | grep "^[0-9]:" | awk '{print "  " $2}' | sed 's/:$//'

# Add successfully loaded modules to /etc/modules for persistence
...
EOF
  ...
  hps_log debug "Created modloop setup script with dependency management"
...
# -----------------------------------
# Create Alpine repository configuration script for TCH boot
#
# Arguments:
#   $1: tmp_dir - Temporary directory for apkovl
#   $2: ips_address - IPS IP address
#   $3: os_id - OS identifier (e.g., "x86_64:alpine:3.20")
#
# Returns:
#   0 on success, 1 on failure
#===============================================================================
_apkovl_create_alpine_repos_script() {
  ...
  hps_log debug "Creating Alpine repository configuration script"
  
  # Get repo path from os_config
  local repo_path=$(get_distro_base_path "$os_id" "relative")
  
  # Create minimal script for repo setup and package install only
  cat > "$tmp_dir/etc/local.d/a-alpine-repos.start" <<EOF
#!/bin/sh
echo "[HPS] Setting up Alpine repositories..."

# Configure repositories using provided repo path
cat > /etc/apk/repositories <<REPOS
${download_base}/apks/main
${download_base}/apks/community
REPOS

# Install required packages for HPS
echo "[HPS] Installing required packages..."
if apk update && apk add --no-cache bash curl; then
    echo "[HPS] Base packages installed"
else
    echo "[HPS] ERROR: Failed to install packages" >&2
    exit 1
fi
echo "[HPS] Alpine repository setup complete"
EOF
  
  chmod +x "$tmp_dir/etc/local.d/a-alpine-repos.start"
  
  hps_log debug "Alpine repository script created with repo path: ${repo_path}"
  return 0
}
