...
initialise_opensvc_cluster() {
  ...
  # --- 2) Get cluster secret and IPS address ---
  cluster_secret="$(remote_cluster_variable OPENSVC_CLUSTER_SECRET 2>/dev/null || true)"
  if [[ -z "$cluster_secret" ]]; then
    remote_log "OPENSVC_CLUSTER_SECRET not found in cluster_config; cannot join cluster"
    return 1
  fi
  
  ips_addr="$(get_provisioning_node 2>/dev/null || true)"
  if [[ -z "$ips_addr" ]]; then
    remote_log "Cannot determine provisioning node address"
    return 1
  fi
  ...
  _osvc_run "set cluster.secret" \
    om cluster set --kw "cluster.secret=${cluster_secret}" || return 1

  _osvc_run "set cluster.nodes" \
    om cluster set --kw "cluster.nodes=${ips_addr}" || return 1

  # Get and set heartbeat type
  local hb_type
  hb_type="$(remote_cluster_variable OSVC_HB_TYPE 2>/dev/null || true)"
  hb_type="${hb_type// /}"
  [[ -z "$hb_type" ]] && hb_type="multicast"
  
  _osvc_run "set hb#1.type=${hb_type}" \
    om cluster set --kw "hb#1.type=${hb_type}" || return 1

  # --- 4) Apply node tags (optional) ---
  if [[ -n "$node_tags" ]]; then
